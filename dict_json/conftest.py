...
def pytest_addoption(parser):
    ...
    parser.addoption("--print", action="store_true",
        help="print all keys")
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--all", action="store", default=False, help="do all types"
    )
    parser.addoption(
        "--print", action="store", default=False, help="print all keys"
    )
...
def debug(request):
    dict_json.logging.basicConfig(level=logging.DEBUG) if request.config.getoption("--debug") else ''
